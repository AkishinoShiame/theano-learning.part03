...
def fit(data, labels, filename = 'weights_v7.pkl'):
    fit_predict(data, labels, filename = filename, action = 'fit') 

def predict(test_dataset, filename = 'weights_v7.pkl' ):
...
def fit_predict(data, labels, action, filename, test_datasets = [], learning_rate=0.1, n_epochs=100, nkerns=[20, 50, 90], batch_size=50, seed=8000):
    ...
    if action=='fit':
        ...
        while(counting < 51):
            ...
            if (counting != 1):
                print 'load file : ', counting
                f = gzip.open(("training_data_200v6_" + str(counting) + ".pkl.gz"), 'rb')
